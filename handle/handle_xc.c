...
/*   Updated: 2018/07/28 14:02:32 by mnakar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../ft_printf.h"

long long	clear_bits_xc(unsigned long long n, int bits)
{
	return ((bits == sizeof(long long) * 8) ? n : ((0x1ull << bits) - 1) & n);
}

int			printhelper_xc(t_info info, long long n, t_help help, int *spaces)
{
	int		len;
	char	*str;
...
int			printformat_xc(t_info info, long long n, int bits, int *spaces)
{
	t_help	help;

	help.spchar = ' ';
	help.sp = 0;
	help.len = (!n && !info.prec) ? 0 : numlen_x(n, 16);
	if (info.zero && !(ISPER(info)))
	{
		...
		help.sp = info.zero - ((help.len > info.prec) ? help.len : info.prec) -
			((ISHASH(info) && n) ? 2 : 0);
	help.bits = bits;
	return (printhelper_xc(info, n, help, spaces));
}
...
void		handle_xc(t_info info, va_list *ap)
{
	...
	if (ISZ(info) && (sz = SIZE_T))
		n = clear_bits_xc(va_arg(*ap, size_t), sz);
	else if (ISJ(info) && (sz = UINTMAX_T))
		n = clear_bits_xc(va_arg(*ap, intmax_t), sz);
	else if (ISLL(info) && (sz = ULLONG))
		n = clear_bits_xc(va_arg(*ap, long long), sz);
	else if (ISL(info) && (sz = ULONG))
		n = clear_bits_xc(va_arg(*ap, long), sz);
	else if (ISH(info) && (sz = USHORT))
		n = clear_bits_xc(va_arg(*ap, int), sz);
	else if (ISHH(info) && (sz = UCHAR))
		n = clear_bits_xc(va_arg(*ap, int), sz);
	else
		n = clear_bits_xc(va_arg(*ap, int), sz);
	len = printformat_xc(info, n, sz, &spaces);
	g_count += spaces + ((len > info.prec) ? len : info.prec) +
		((ISHASH(info) && clear_bits_xc(n, sz)) ? 2 : 0);
}
